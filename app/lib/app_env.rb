  ...
  def self.ensure!
    ...
    unless missing_required_keys.empty?
      if Rails.env.production?
        ...
        Rails.logger.warn Rainbow("WARNING:").red + " Missing ENV variables " + Rainbow(missing_required_keys.join(",")).red + ". Not all keys are necessary in development. Any features that depend on this key will not work."
      end
    end

    missing_optional_keys = OPTIONAL_KEYS - ENV.keys
    unless missing_optional_keys.empty?
      Rails.logger.info Rainbow("Missing optional ENV variables #{missing_optional_keys.join(",")}. Any features that depend on these keys will not work.").yellow
    end
  end
