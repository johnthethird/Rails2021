    ...
    //====================================================================
    {
      ...
      _parser.addGrammarElement("postfixExpression", function (parser, tokens) {
        ...
        if (tokens.matchOpToken(":")) {
          ...
          return {
            ...
            transpile: function () {
              return (
                "_hyperscript.runtime.typeCheck(" +
                parser.transpile(root) +
                ", '" +
                typeName.value +
                "', " +
      ...
      _parser.addGrammarElement("target", function (parser, tokens) {
        ...
        return {
          ...
          transpile: function () {
            return (
              "_hyperscript.runtime.evalTarget(" +
              parser.transpile(root) +
              ", [" +
              propPath
                .map(function (prop) {
        ...
        var eventListener = {
          ...
          transpile: function () {
            return (
              "(function(me){" +
              "var my = me;\n" +
              "_hyperscript.runtime.forEach( " +
              parser.transpile(from) +
              ", function(target){\n" +
        ...
        if (tokens.matchToken("wait")) {
          ...
          return {
            ...
            transpile: function () {
              ...
              return (
                "setTimeout(function () { " +
                parser.transpile(capturedNext) +
                " }, " +
                parser.transpile(this.time) +
                ")"
              );
            },
          };
          ...
          return {
            ...
            transpile: function () {
              return (
                ...
                parser.transpile(to) +
                ", function (target) {" +
                "  _hyperscript.runtime.triggerEvent(target, '" +
                parser.transpile(eventName) +
                "'," +
                parser.transpile(details, "{}") +
                ")" +
                "})"
              );
            },
      ...
      _parser.addGrammarElement("triggerCmd", function (parser, tokens) {
        if (tokens.matchToken("trigger")) {
          ...
          return {
            ...
            transpile: function () {
              return (
                "_hyperscript.runtime.triggerEvent(me, '" +
                parser.transpile(eventName) +
                "'," +
                parser.transpile(details, "{}") +
                ");"
          ...
          return {
            ...
            transpile: function () {
              ...
              return (
                "  _hyperscript.runtime.forEach(" +
                parser.transpile(from) +
                ", function (target) { target.classList.remove('" +
                clazz +
                "') }); " +
                "_hyperscript.runtime.forEach( " +
                parser.transpile(forElt) +
                ", function (target) {" +
                "  target.classList.add('" +
                clazz +
                "')" +
            ...
            transpile: function () {
              if (this.symbolWrite) {
                ...
                if (this.op === "into") {
                  ...
                  return (
                    "_hyperscript.runtime.forEach( " +
                    parser.transpile(target) +
                    ", function (target) {" +
                    "  target." +
                    lastProperty +
                    "=" +
                    parser.transpile(value) +
                    "})"
                  );
          ...
          return {
            ...
            transpile: function () {
              return (
                ...
                "){" +
                "" +
                parser.transpile(trueBranch) +
                "}" +
                "   else {" +
                parser.transpile(falseBranch, "") +
                "}"
              );
            },
          };
